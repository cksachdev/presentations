...
<Image src={require('file-loader!./img/vljs.png')} />

## Hello there ValenciaJS friends! 👋🏼
...
## !sdneirf SJaicnelaV Ereht OlleH 👋🏼
...
const reverseWord = word => compose(join(''), reverse, split(''))(word)
```
...
<Appear>
  ...
  <p><small>The container is also a function!</small></p>
  <p>Implements a `map` function to apply other functions to the data</p>
  <p><small>`map` is not about iteration but about applying functions to data</small></p>
</Appear>
...
const transformWords = map(compose(capitalizeFirstLetter, reverseWord))
const translate = compose(join(' '), reverse, transformWords, split(' '))
```
